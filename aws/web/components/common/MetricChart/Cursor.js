...
function MetricCursor({ data, cursorVisible, cursorX, margin, height, width, y, units, type }) {
    return (
        <Cursor
            y={margin.top}
            x={cursorX}
...
function label(type, data, units) {
    ...
    if (data) {
        const value = Math.round(data[type] * 100) / 100;
